 ...
 * @private
 */
Queue.prototype._touch = function() {
  ...
  this.qKeys.forEach(function(key) {
    _this.connection.expire(key, _this._ttl, function(err, resp) {
      if (err) {
  ...
  // if this queue is also globally discoverable, notify all other bus peers that the queue has been touched
  if (this._discoverable) {
    if (!this.notificationPubsub) {
      this.notificationPubsub = this.bus.pubsub(this.bus.notificationsChannel);
    }
    this.notificationPubsub.publish({event: 'queue', key: this.metadataKey, ttl: this._ttl}, function(err) {
      if (err) {
        _this.emit('error', "error notifying touched event: " + err);
      }
    });
  }
};
...
/**
 * Find the location of a queue - local bus or federated bus
 * @param cb
 */
Queue.prototype.find = function(cb) {
  var _this = this;
  this._connect(function() {
    _this.metadata(function(err, values) {
      if (err) {
        cb && cb('error finding queue: ' + err);
        return;
      }
      cb && cb(null, values && values.source || null);
    });
  });
};
  ...
  function _attach() {
    ...
    _this.ttl(function(err, ttl) {
      ...
      if (!ttl) {
        ...
        _this.metadata({ttl: _this._ttl, source: 'local', discover: _this._discoverable}, function(err) {
          if (err) {
            _this.emit('error', err);
            return;
          }
...
Queue.prototype.metadata = function(name, value, cb) {
  ...
  // old behavior. value must be an object
  if (value) {
    var v = value;
    value = {};
    value[name] = v;
  ...
  if (value) {
    var arr = [this.metadataKey];
    Object.keys(value).forEach(function(k) {
      arr.push(k);
      arr.push(value[k]);
    });
    this.connection.hmset(arr, function(err, resp) {
      if (err) {
    ...
    if (name) {
      ...
      this.connection.hgetall(this.metadataKey, function(err, resp) {
        if (err) {
          cb && cb("error reading all metadata: " + err);
          return;
        }
        cb && cb(null, resp);
      });
...
Queue.prototype.promisify = function() {
  return this.bus.promisify(this, ["find", "ttl", "metadata", "exists", "count", "flush", "push", "ack"]);
};
